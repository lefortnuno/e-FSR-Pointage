...
import PointageEntree from "../../components/modals/pointages/entree";
import {
  BsSearch,
  BsArrowLeftRight,
...
export default function PointageDesAbsent() {
  //#region //-variable
  ...
  function getHisto() {
    ...
    axios
      .post(url_req + "absent/byDay/", data, u_info.opts)
  ...
  function rechercheElement(event) {
    ...
    if (!valeur) {
      ...
      axios
        .post(url_req + `absent/rechercheParJour/`, finalInputs, u_info.opts)
        .then((response) => {
          if (
            response.status === 200 &&
            response.data.success &&
