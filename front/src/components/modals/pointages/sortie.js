...
export default function PointageSortie({ userData, onClose, onSave, u_info }) {
  const handleSave = () => {
    const data = { employeIm: userData.im };
    axios
      .put(`pointage/web/sortie/`, data, u_info.opts)
      .then((response) => {
  ...
  return (
    <Modal show onHide={onClose} centered className="custom-modal">
      <Modal.Header closeButton className="bg-secondary">
        <Modal.Title className="text-white">Pointage de Sortie</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form>
